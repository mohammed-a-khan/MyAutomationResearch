...
async function createBrowserSession(config) {
    try {
        ...
        switch (config.browserType) {
            case 'chromium':
                browser = await playwright_1.chromium.launch({ headless: config.headless });
                break;
            case 'firefox':
                browser = await playwright_1.firefox.launch({ headless: config.headless });
                break;
            case 'webkit':
                browser = await playwright_1.webkit.launch({ headless: config.headless });
                break;
            case 'msedge':
        ...
        if (config.environmentVariables) {
            await context.setExtraHTTPHeaders(Object.entries(config.environmentVariables).reduce((acc, [key, value]) => {
                acc[`X-Env-${key}`] = value;
                return acc;
            }, {}));
        }
...
// Set viewport size
app.post('/browser/viewport', async (req, res) => {
    try {
...
app.post('/browser/inject', async (req, res) => {
    try {
...
app.post('/browser/element-screenshot', async (req, res) => {
    try {
...
app.post('/browser/info', async (req, res) => {
    try {
        const { sessionId } = req.body;
        if (!sessionId) {
            return res.status(400).json({ error: 'Missing sessionId parameter' });
...
app.post('/browser/network-capture', async (req, res) => {
    try {
...
app.post('/browser/console-capture', async (req, res) => {
    try {
        const { sessionId, enabled } = req.body;
        if (!sessionId || enabled === undefined) {
            return res.status(400).json({ error: 'Missing required parameters' });
...
app.post('/browser/ping', async (req, res) => {
    try {
        const { sessionId } = req.body;
        if (!sessionId) {
            return res.status(400).json({ error: 'Missing sessionId parameter' });
